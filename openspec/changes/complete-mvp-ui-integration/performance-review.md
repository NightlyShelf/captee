...
- Finding: The post-selection review is a borderless transparent monitor-sized floating GTK surface moved to the Hyprland workspace and monitor that were active before selection. It retains optional selection geometry and a stroked selection frame until the user confirms, discards, or modifies it. The Hyprland fallback also retains one valid full-monitor PNG snapshot taken before `slurp`; this is used only for visual continuity under the dim layer and is not the selected capture image.
- Impact: The GTK review adds one temporary top-level surface, one editable source buffer, short context string, one lightweight selection frame, and one decoded desktop texture while visible. The fallback adds one bounded monitor screenshot and temporarily retains its encoded bytes alongside the selected PNG; the floating surface does not request compositor fullscreen state, so the existing workspace windows remain present underneath. Placement adds one bounded active-workspace query before capture and at most 20 short window-list/dispatch retry cycles after mapping. Annotation insertion creates one additional source string proportional to the annotation and image expression, while PNG validation and storage remain linear in image size.
