...
- Mitigation: The worker ensures the checked-in lowercase desktop entry is present in the user application-data directory when running outside a package, then registers `com.nightlyshelf.captee` as a host app on the same D-Bus connection before creating the shortcut session. This avoids the portal's missing-app-info rejection for unsandboxed development launches. The one-time directory/file check and bounded metadata write happen once at startup; registration failures are surfaced as status text, activation still passes through the normal project/busy/cancellation checks, and the existing capture worker remains responsible for screenshot subprocess bounds.
